...
public class H09_TestUtilsP {
    ...
    public static Predicate<Type> matchWildcard(boolean isUpperBound, Type... expectedBounds) {
        return new GenericPredicate(
            type -> {
                ...
                return bounds.getLeft().size() == expectedBounds.length &&
                    bounds.getLeft().stream()
                        .map(Type::getTypeName)
                        .allMatch(bound ->
                            Stream.of(expectedBounds)
                                .map(Type::getTypeName)
                                .anyMatch(bound::equals)
                        );
            },
            String.format(
                "Wildcard: ? %s %s",
    ...
    /**
     * Creates a new Predicate that matches a Type if it is a wildcard type and has no bounds.
     *
     * @return a Predicate that matches a wildcard type with no bounds.
     */
    public static Predicate<Type> matchWildcardNoBounds() {
        return new GenericPredicate(
            type -> {
                Pair<List<Type>, List<Type>> bounds = getBounds(type);
                if (!(type instanceof WildcardType wildcardType)) {
                ...
                return bounds.getLeft().isEmpty() && bounds.getRight().size() == 1;
            },
            "Wildcard: ?"
        );
    }
    ...
    public static Predicate<Type> matchNested(Type outerType, Predicate<Type>... expectedNested) {
        return new GenericPredicate(
            ...
            String.format("Type: %s; Inner Type: %s", outerType.getTypeName(), List.of(expectedNested))
        );
    }
     ...
     *
     * @param type the type to retrieve the Bounds from
    ...
    /**
     * Asserts that the given class is generic.
     *
     * @param toTest a class reference to the class that should be checked.
     */
    public static void assertGeneric(Class<?> toTest) {
        assertTrue(
            !getDefinedTypes(toTest, ".*").isEmpty(),
            emptyContext(),
            r -> String.format("The class %s() is not Generic.", toTest.getName())
        );
    }
